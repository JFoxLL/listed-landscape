...
namespace :db do
  ...
  task import_key_people_231122: :environment do
    ...
    original_content = File.read("db/import_data/key_people_231122.csv", encoding: "UTF-8")
    
    # Remove invalid UTF-8 characters
    cleaned_content = original_content.encode('UTF-8', 'binary', invalid: :replace, undef: :replace, replace: '')
    
    # Write the cleaned content to a new file
    File.open("db/import_data/key_people_231122.csv", "w:UTF-8") { |file| file.write(cleaned_content) }
    
    # Import new records
    puts "Importing new key people..."
    CSV.foreach("db/import_data/key_people_231122.csv", headers: true) do |row|
      KeyPerson.create!(
